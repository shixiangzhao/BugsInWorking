	...
	public void prod(int index) {
		synchronized (this) {
			...
			while (foods.size() > Max) {
				System.out.println(Thread.currentThread().getName() + "通知所有人，食材够了");
				this.notifyAll();// 这个唤醒是针对生产者和消费者，有all
				try {
					this.wait();// 这个唤醒是针对生产者，没有all
					String name = Thread.currentThread().getName();
					System.out.println(name + "生产者被唤醒");
	...
	public void consu(int index) {
		synchronized (this) {
			while (foods.size() < index) {
				System.out.println(Thread.currentThread().getName() + "通知所有人，食材不够了");
				this.notifyAll();// 这个唤醒是针对生产者和消费者，有all
				try {
					String name = Thread.currentThread().getName();
					this.wait();// 这个唤醒是针对消费者，没有all
					System.out.println(name + "消费者被唤醒");
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			}
